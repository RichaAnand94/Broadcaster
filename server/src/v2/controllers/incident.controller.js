...
const createRedFlag = async (req, res) => {
    if (!req.body.title || !req.body.comment || !req.body.location) {
        return res.status(400).send({ 'status': 400, error: 'Missing mandatory fields' })
    } else {
...
const getIncidentsById = async (req, res) => {
    ...
    if (incident) {
        ...
        return res.status(404).send({ 'status': 404, error: 'No data found for id: ' + req.params.id });
    }
}
    ...
    if (incident) {
        if (incident.created_by == req.headers.userId) {
            if (incident.status === 'pending') {
                ...
                return res.status(400).send({ 'status': 400, error: 'This red-flag can not be deleted' });
            }
        } else {
    ...
    if (incident) {
        if (incident.created_by == req.headers.userId) {
            if (incident.status === 'pending') {
                ...
                return res.status(400).send({ 'status': 400, error: 'This intervention can not be deleted' });
            }
        } else {
